...
import com.github.sofn.dagflow.executor.DagFlowDefaultExecutor;
import lombok.Data;
...
public class Node<C extends DagFlowContext, T extends DagFlowCommand<C, ?>> {
     ...
     * 依赖，保存顺序
     */
    private Set<Node<C, ?>> depends = new LinkedHashSet<>();
    ...
    public Node(String name, Class<T> clazz) {
        this.name = name;
        this.clazz = clazz;
    }

    public Node(String name, T instance) {
        this.name = name;
        this.instance = instance;
    }

    public boolean addDepend(Node<C, ?> depend) {
        return this.depends.add(depend);
    }

    public boolean addDepends(Collection<Node<C, ?>> depends) {
        return this.depends.addAll(depends);
    }

    public boolean addDepends(Node<C, ?>[] depends) {
